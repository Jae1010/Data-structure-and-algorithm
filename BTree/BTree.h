...
template < typename T> //关键码插入后后若节点上溢，则做节点分裂处理
void BTree<T>::solveOverflow(BTNodePosi(T) v) {
	...
	p->child.insert(r + 1, u); u->parent = p; //新节点u不父节点p互联
...
template <typename T> bool BTree<T>::remove(const T & e) { //从BTree树中删除关键码e
	...
	if (v->child[0]) { //若v非叶子，则e的后继必属亍某叶节点
		BTNodePosi(T) u = v->child[r + 1]; //在右子树中一直向左，即可
		while (u->child[0]) u = u->child[0]; //找出的后继
		v->key[r] = u->key[0]; v = u; r = 0; //并与之交换位置
	} //至此， v必然位亍最底局，且其中第r个关键码就是待删除者
...
void BTree<T>::solveUnderflow(BTNodePosi(T) v) {
	...
	// 情况3：左、右兄弟要么为空（但不可能同时），要么都太“瘦” ——合并
		...
		solveUnderflow(p); //上升一层，如有必要则继续分裂——至多递归O(logn)局
		return;
}